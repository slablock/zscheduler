...
public class JsonConfigurator {
    private static final Logger log = LoggerFactory.getLogger(JsonConfigurator.class);
    ...
    public <T> T configurate(
        ...
        for (String prop : props.stringPropertyNames()) {
            if (prop.startsWith(propertyBase)) {
                ...
                try {
                    ...
                    log.info("Unable to parse [{}]=[{}] as a json object, using as is.", prop, propValue, e);
                    value = propValue;
                }
        ...
        log.debug("Loaded class[{}] from props[{}] as [{}]", clazz, propertyBase, config);
